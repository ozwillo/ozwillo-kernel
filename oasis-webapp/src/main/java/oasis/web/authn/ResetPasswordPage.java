...
public class ResetPasswordPage {
  ...
  public Response get() {
    ...
    if (changePasswordToken == null) {
      return ForgotPasswordPage.form(Response.status(Response.Status.NOT_FOUND), ForgotPasswordPage.ForgotPasswordError.EXPIRED_LINK);
    }
  ...
  public Response post(
    ...
    if (Strings.isNullOrEmpty(newpwd)) {
      return form(Response.status(Response.Status.BAD_REQUEST), account, ResetPasswordError.MISSING_REQUIRED_FIELD);
    }
  ...
  private Response form(Response.ResponseBuilder builder, UserAccount account, @Nullable ResetPasswordError error) {
    return builder
        .header(HttpHeaders.CACHE_CONTROL, "no-cache, no-store")
        .header("Pragma", "no-cache")
            // cf. https://www.owasp.org/index.php/List_of_useful_HTTP_headers
